...
import com.cn.mv.R;
import com.cn.mv.adapter.FileVideoAdapter;
...
public class FragmentVideo extends Fragment {
    ...
    private void bindView() {
        recyclerView = (RecyclerView) v.findViewById(R.id.video_rv);
        tvZdSearch = (TextView) v.findViewById(R.id.video_zd_search);
    ...
    private synchronized void initView(int modType) {
        ...
        if (fileLis != null && fileLis.size() > 0) {
            adapter = new FileVideoAdapter(getContext(), fileLis, modType);
    ...
    private FileVideoAdapter.OnItemUpdateClickListener itemUpdateClickLis = new FileVideoAdapter.OnItemUpdateClickListener() {
        @Override
    ...
    private FileVideoAdapter.OnItemDelClickListener onItemDelClickLis = new FileVideoAdapter.OnItemDelClickListener() {
        @Override
    ...
    private FileVideoAdapter.OnItemClickListener itemClickLis = new FileVideoAdapter.OnItemClickListener() {
        @Override
        public void onItemClick(View v, int position, File file) {
            if (adapter.getCheckFile().size() != 0) {
                if (file != null) {
                    CheckBox cb = (CheckBox) v.findViewById(R.id.video_cb);
                    //设置选中状态为其当前状态的相反状态
    ...
    private FileVideoAdapter.OnItemLongClickListener longClickLis = new FileVideoAdapter.OnItemLongClickListener() {
        @Override
    ...
    public void fileOrder() {
        tvZdSearch.setVisibility(View.VISIBLE);
        fileLis = FileUtil.getChildrenFiles(fileLis, SharedPreferencesUtils.getVideoOrderType(getContext()));
    ...
    public void filesMod() {
        initView(SharedPreferencesUtils.getVideoModType(getContext()));
    }
    ...
    public static void openFile(File file, Context context) {
        ...
        if (list != null && list.size() > 0) {
            ...
            Toast.makeText(context, "无法打开该视频！", Toast.LENGTH_SHORT).show();
        }
    }
